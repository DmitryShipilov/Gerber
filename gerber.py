...
class Drill:
	...
	def parse(self):
		...
		for drill in self.drill_lines:
			...
			if(drill_data):
				...
				self.drill_types[int(drill_data.group(1))] = float(drill_data.group(2))*float(self.drill_unit)
			if(drill_num):
				...
				if int(drill_num.group(1)) > 0:
					...
					self.drill_d=float(self.drill_types[int(drill_num.group(1))])
	...
	def parse_drill_g(self,g_num,drill):
		...
		if (g_num ==0):
			print ("G00:Route Mode is not supported:" + drillF)
		if (g_num ==1):
...
class Gerber:
	...
	def parse(self):
		...
		for line in self.gerber_lines:
			...
			#Find M
			if(line.find("M") == 0):
				print ("M comand is ignored:" + line)
	...
	def parse_d(self,d,x,y):
		...
		#move
			...
			if (self.arc_interpolation):
				...
				if (self.arc_interpolation360 and (self.pre_x == x and self.pre_y == y)):
					...
					#circle_points(c,r,ccw,kaku)
					self.tmp_points.extend(circle_points((self.pre_x+self.ii,self.pre_y+self.jj),r,self.arc_dir,self.CirPointN))
					#Path:
					#		def __init__(self, fig_type, w, points, polygon, active = 1):
					return
		...
		self.pre_dnum = d
def circle_points(c,r,ccw,kaku):
	points=[]
	if ccw:
		start_angle=0.0
		ang_step=(2*math.pi)/(kaku-1)
	else:
		start_angle=2*math.pi
		ang_step=-(2*math.pi)/(kaku-1)
	i = 0
	while i < kaku:
		cir_x=c[0]+r*math.cos(start_angle+ang_step*float(i))
		cir_y=c[1]+r*math.sin(start_angle+ang_step*float(i))
		points.append((cir_x,cir_y))
		i += 1
	return points
