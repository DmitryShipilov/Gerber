...
class Gerber_OP:
	...
	def drill2shapely(self,select_d=0,tool_type=0):
	...
	def minimum(self,maxx,maxy,minx,miny):	#Ver. 5
		out = box(minx, miny, maxx, maxy)
		merge = []
		for elmt in self.tmp_figs.elements:
			if elmt.active == 0:
				continue
			if elmt.element.is_empty:
				continue
			if elmt.element.geom_type == 'Polygon':
				elmt.element=elmt.element.simplify(self.ref_length)
				merge.append(elmt.element)
			elif elmt.element.geom_type == 'MultiPolygon':
				for polygon in elmt.element:
					polygon=polygon.simplify(self.ref_length)
					merge.append(polygon)
		tmp = cascaded_union(merge)
		tmp = out.difference(tmp)
		if tmp.geom_type=='MultiPolygon':
			for poly in tmp:
				if len(poly.interiors)==1:
					dist=poly.exterior.distance(poly.interiors[0])
					tmppoly=Polygon(poly.interiors[0]).buffer(dist/2)
					self.figs.add(self.Figs(tmppoly))
				if len(poly.interiors)>1:
					for i in range(len(poly.interiors)):
						min_dist=10e9
						if min_dist > poly.exterior.distance(poly.interiors[i]):
							min_dist = poly.exterior.distance(poly.interiors[i])
						for j in range(len(poly.interiors)):
							if i==j:
								continue
							if min_dist > poly.interiors[i].distance(poly.interiors[j]):
								min_dist = poly.interiors[i].distance(poly.interiors[j])
						tmppoly=Polygon(poly.interiors[i]).buffer(min_dist/2)
						#self.figs.add(self.Figs(Polygon(tmppoly.exterior)))
						self.figs.add(self.Figs(Polygon(tmppoly)))
	def voronoi(self,maxx,maxy,minx,miny):		#Ver. 2
		#Check:Is "scipy" module installed? 
		try:
			imp.find_module('scipy')
			import poly_voro
		except ImportError:
			print ("scipy (for voronoi) is not installed")
			exit(1)
		out = box(minx, miny, maxx, maxy)
		merge = []
		for elmt in self.tmp_figs.elements:
			...
			if elmt.element.is_empty:
				continue
			if elmt.element.geom_type == 'Polygon':
				elmt.element=elmt.element.simplify(self.ref_length,preserve_topology=True)
				merge.append(elmt.element)
			elif elmt.element.geom_type == 'MultiPolygon':
				for polygon in elmt.element:
					polygon=polygon.simplify(self.ref_length,preserve_topology=True)
					merge.append(polygon)
		tmp = cascaded_union(merge)
		tmp = out.difference(tmp)
		all_ridges=[]
		...
		if tmp.geom_type=='MultiPolygon':
			print ("****MultiPolygon****")
			for poly in tmp:
				pv=poly_voro.poly_voro(poly)
				all_ridges.extend(pv.ridges)
		elif tmp.geom_type=='Polygon':
			print ("****Polygon****")
			pv=poly_voro.poly_voro(poly)
			all_ridges.extend(pv.ridges)
		...
		for ridge in all_ridges:
			lines.append(LineString(ridge))
		multiline=linemerge(lines)
		for line in multiline:
			self.figs.add(self.Figs(line))
